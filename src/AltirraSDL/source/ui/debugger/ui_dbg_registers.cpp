...
bool ATImGuiRegistersPaneImpl::Render() {
	...
	if (mbStateValid && !mFormattedState.empty()) {
		mEditBuffer.assign(mFormattedState.begin(), mFormattedState.end());
		mEditBuffer.push_back('\0');

		ImGui::InputTextMultiline("##RegistersText", mEditBuffer.data(),
			mEditBuffer.size(), ImGui::GetContentRegionAvail(),
