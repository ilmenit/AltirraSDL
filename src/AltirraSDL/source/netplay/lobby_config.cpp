...
const char* const kDefaultLobbyIni =
	...
	"enabled = true\n"
	"\n"
	"; Backup lobby — same server, alternate DNS via DuckDNS.  For native\n"
	"; (desktop) builds this is enabled by default so federation auto-\n"
	"; falls-back when one DNS is unreachable (or its TLS cert is pending);\n"
	"; sessions are deduplicated by sessionId so Browse stays clean.\n"
	"; For the WASM (browser) build this entry is disabled by default,\n"
	"; because DuckDNS is on most ad-blocker / privacy-extension blocklists\n"
