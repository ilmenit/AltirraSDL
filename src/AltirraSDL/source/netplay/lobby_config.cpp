...
const char* const kDefaultLobbyIni =
	...
	"enabled = true\n"
	"\n"
	"; Backup lobby — same server, alternate DNS via DuckDNS.  Active\n"
	"; by default so federation auto-falls-back: when one DNS is\n"
	"; unreachable (or its TLS cert is pending) the other still answers,\n"
	"; sessions are deduplicated by sessionId so Browse stays clean.\n"
	"; Trade-off: while BOTH names resolve to our server, hosting a\n"
	"; game registers it on each lobby separately — the same physical\n"
	"; session appears twice in Browse (different sessionIds, same\n"
	"; backend).  That's cosmetic and joiners can use either copy; flip\n"
